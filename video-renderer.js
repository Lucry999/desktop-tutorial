  ...
  async function renderVideo(){
    ...
    try{
      ...
      if(!response.ok){
        ...
        throw new Error(data.error||'Qualitäts-Render konnte nicht gestartet werden');
      }
      ...
      for(let attempt=0;attempt<120;attempt++){
        ...
        const poll=await fetch(API_BASE+'/api/video-hq/'+encodeURIComponent(job.jobId),{cache:'no-store'});
        if(!poll.ok) continue;
        status=await poll.json();
        const progress=Math.max(0,Math.min(100,Number(status.progress)||0));
        if(btn)btn.textContent='🎬 KI-Video wird gebaut… '+progress+'%';
        if(hint)hint.textContent='Echte KI-Szenen werden erstellt und anschließend mit Stimme und Captions zusammengesetzt.';
      ...
      if(!status||status.status!=='done'||!status.url)throw new Error('Der Renderjob läuft ungewöhnlich lange. Bitte später erneut öffnen.');
      lastUrl=API_BASE+status.url;
      video.src=lastUrl;
      wrap.classList.remove('hidden');
      demo.classList.add('hidden');
      hint.textContent='Fertiges 9:16-Video mit echten KI-Szenen, KI-Stimme und Captions.';
      ...
      if(result)result.innerHTML+='<br><br><b>✅ Qualitätsvideo fertig!</b><br><span>Echte KI-Szenen wurden erzeugt und zusammengesetzt.</span>';
      if(btn)btn.textContent='🎬 Video erneut erstellen';
    }catch(error){
      console.error(error);
      if(result)result.innerHTML+='<br><br><b>⚠️ Video-Fehler:</b> '+escape(error.message||'Unbekannter Fehler');
